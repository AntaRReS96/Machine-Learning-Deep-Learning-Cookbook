from config import BLOCK_SIZE, WIN_NN_WIDTH, WIN_NN_HEIGHT, DISPLAY_SPLIT, GAME_SPEED
from display_nn import GameDisplay  
import pygame
...
class Snake:
    ...
    def is_collided(self, block=None):
        ...
        if block.x < 0 or block.x >= WIN_WIDTH or block.y < 0 or block.y >= WIN_NN_HEIGHT:
            return True
        
        for body_block in self.snake_elements[1:]:
            if block.x == body_block.x and block.y == body_block.y:
                return True
        return False
...
class SnakeGameAI:
    ...
    def play_step(self, action, activations):
        ...
        self.display.update_display(self.snake, self.apple, self.model, activations)  
        self.clock.tick(GAME_SPEED)

        return self._get_state(), reward, self.game_over, self.score

    def _get_state(self):
        """Zwraca stan gry jako wektor."""
