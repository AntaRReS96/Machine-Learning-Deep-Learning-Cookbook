import pygame
from snake_config import BLOCK_SIZE, WIN_NN_WIDTH, WIN_NN_HEIGHT, DISPLAY_SPLIT, \
                   SNAKE_HEAD_COLOR, SNAKE_COLOR, BLACK, GRAY, RED, \
                   WEIGHT_POS_COLOR, WEIGHT_NEG_COLOR

# Add WHITE color for score text
WHITE = (255, 255, 255)
...
class GameDisplay:
    ...
    def draw_score(self, score):
        """Rysowanie wyniku w prawym górnym rogu planszy gry."""
        score_text = self.font.render(f"Score: {score}", True, WHITE)
        # Position in the top-right corner of the game area
        score_x = DISPLAY_SPLIT + 10  # 10 pixels from the left edge of game area
        score_y = 10  # 10 pixels from the top
        self.display.blit(score_text, (score_x, score_y))

    def update_display(self, snake, apple, nn_model, activations, score):
        """Aktualizacja ekranu gry i rysowanie sieci neuronowej wraz z aktywacjami."""
        self.draw_background()
        self.draw_snake(snake)
        self.draw_apple(apple)
        self.draw_score(score)  # Add score display
