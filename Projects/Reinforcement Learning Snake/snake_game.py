from snake_config import BLOCK_SIZE, WIN_WIDTH, WIN_HEIGHT, GAME_SPEED
from snake_display import GameDisplay  
import pygame
...
class SnakeGameAI:
    ...
    def reset(self):
        """Resetuje stan gry i zwraca początkowy stan."""
    ...
    def play_step(self, action, epsilon=None, game_count=None):
        ...
        # Główna kara za śmierć
        if self.snake.is_collided():
            self.game_over = True
            reward = -10  # Zmniejszona kara za śmierć
            return self._get_state(), reward, self.game_over, self.score

        # Główna nagroda za zjedzenie jabłka
        if self.snake.head[0] == self.apple.x and self.snake.head[1] == self.apple.y:
            self.score += 1
            reward = 10  # Nagroda za jabłko
            self.apple.change_pos()
        else:
            self.snake.snake_elements.pop()

        # Nagroda za przeżycie (motywuje do unikania śmierci)
        reward += 0.1

        # Nagroda za zbliżanie się do jabłka
        new_distance = self.snake.get_distance_from_apple(self.apple)
        if new_distance < old_distance:
            reward += 0.5  # Nagroda za zbliżanie się
        else:
            reward -= 0.3  # Kara za oddalanie się

        # Dodatkowa kara za zbyt długie krążenie (anti-loop)
        if len(self.snake.snake_elements) > 3:
            head_x, head_y = self.snake.head
            # Sprawdź czy węż jest zbyt daleko od jabłka przez długi czas
            if abs(head_x - self.apple.x) + abs(head_y - self.apple.y) > WIN_WIDTH * 0.8:
                reward -= 0.1

        self.display.update_display(self.snake, self.apple, self.score, epsilon, game_count)
    ...
    def _get_state(self):
        """
        Ulepszona reprezentacja stanu z dodatkową informacją o ciele węża i odległości.
        """
        ...
        # Obliczanie odległości od jabłka (znormalizowana)
        apple_distance = abs(self.apple.x - head[0]) + abs(self.apple.y - head[1])
        max_distance = WIN_WIDTH + WIN_HEIGHT
        normalized_distance = apple_distance / max_distance

        # Dodanie informacji o najbliższych segmentach ciała
        body_danger_front = 0
        body_danger_right = 0
        body_danger_left = 0
        
        for segment in self.snake.snake_elements[1:]:  # Pomijamy głowę
            if segment == front_block:
                body_danger_front = 1
            elif segment == right_block:
                body_danger_right = 1
            elif segment == left_block:
                body_danger_left = 1
        ...
        state = [
            ...
            normalized_distance,          # Znormalizowana odległość od jabłka
            body_danger_front,           # Czy ciało jest prosto
            body_danger_right,           # Czy ciało jest po prawej
            body_danger_left,            # Czy ciało jest po lewej
            len(self.snake.snake_elements) / 100.0,  # Długość węża (znormalizowana)
        ]
